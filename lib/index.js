...
function flushEffects(fiber) {
  if (fiber.effects) {
    ...
    for (var i = 0; i < fiber.effects.length; i++) {
      if (!fiber.alternate || !isArrEleSame(fiber.effects[i].dependiencies, oldFiber.effects[i].dependiencies)) {
...
var isArrEleSame = function isArrEleSame(a, b) {
  var res = true;
...
function Counter() {
  ...
  var _Didact$useState3 = Didact.useState(1),
      ...
      counter = _Didact$useState4[0],
      setCounter = _Didact$useState4[1];

  Didact.useEffect(function () {
  ...
  }, "Count: state ", state, ", counter ", counter), Didact.createElement("h1", {
    onClick: function onClick() {
